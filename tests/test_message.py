...
from fastapi_mailer.msg import MailMsg
...
def test_initialize():
    message = MessageSchema(
        subject='test subject', recipients=['uzezio22@gmail.com'], body='test', subtype='plain'
    )
...
def test_recipients_properly_initialized():
    message = MessageSchema(subject='test subject', recipients=[], body='test', subtype='plain')
...
def test_bcc():
    msg = MessageSchema(subject='subject', recipients=[], bcc=['bcc@example.com'])
...
def test_replyto():
    msg = MessageSchema(subject='subject', recipients=[], reply_to=['replyto@example.com'])
...
def test_cc():
    msg = MessageSchema(subject='subject', recipients=[], cc=['cc@example.com'])
...
def test_multipart_subtype():
    message = MessageSchema(
        subject='test subject', recipients=['to@example.com'], body='test', subtype='plain'
    )
